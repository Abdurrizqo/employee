...
function getRanting() {
    ...
    async function fetchRantings() {
        try {
            ...
            rantings.forEach((ranting, index) => {
                ...
                <td class="px-6 py-4 text-center ${ranting.is_active ? 'text-green-500' : 'text-red-500'} ">${ranting.is_active ? 'Aktif' : 'Tidak Aktif'}</td>
                <td class="px-6 py-4 text-center">
                    <div class="inline-flex rounded-md shadow-sm items-center justify-center w-full">
                        <button class="px-8 py-2 text-sm font-medium text-white bg-green-600 border rounded-s-lg hover:bg-green-500 focus:z-10 focus:ring-2">
                            Edit
                        </button>
                        <button class="px-8 py-2 text-sm font-medium text-white bg-red-600 border rounded-e-lg hover:bg-red-500">
                            Delete
                        </button>
                    </div>
                </td>
    ...
    async function fetchRantingsWithSearch(search) {
        try {
            // Fetch data dari endpoint menggunakan Axios
            const response = await axios.get(`/data-ranting/all?search=${search}`);
            const rantings = response.data.data;
            ...
            rantings.forEach((ranting, index) => {
                ...
                <td class="px-6 py-4 text-center">${ranting.nama_ranting}</td>
                <td class="px-6 py-4 text-center">${ranting.is_active ? 'Aktif' : 'Tidak Aktif'}</td>
                <td class="px-6 py-4 text-center">
                    <div class="inline-flex rounded-md shadow-sm items-center justify-center w-full">

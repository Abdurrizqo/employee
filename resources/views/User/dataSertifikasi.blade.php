        ...
        <div class="border rounded-lg p-3 w-full bg-white mb-3" id="containerAdd">
            <form id="addForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label for="penyelenggara" class="block mb-2 text-sm font-medium text-gray-900">Penyelenggara<span
                    ...
                    <div>
                        <label for="sertifikasi" class="block mb-2 text-sm font-medium text-gray-900">Sertifikasi<span
                                class="text-red-400">*</span></label>
                        <input type="text" id="sertifikasi" name="sertifikasi"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="Sertifikasi" required />
                        @error('sertifikasi')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 " for="file_input">Bukti
                            Sertifikat</label>
                        <input
                            class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50"
                            aria-describedby="file_input_help" id="file_input" type="file" name="dokumen_sertifikasi"
                    ...
                    <tbody id="adminTableBody">
                        @foreach ($sertifikasi as $item)
                            <tr class="odd:bg-white">
                                ...
                                <td class="px-6 py-4 text-center">{{ $item->sertifikasi }}</td>
                                <td class="px-6 py-4 text-center">{{ $item->tahun }}</td>
                                <td class="px-6 py-4 text-center"><a target="#"
                                        href="/dokumen-sertifikasi/{{ $item->id }}"
                                        class="text-violet-500 hover:text-violet-600 hover:underline">Dokumen
                                        Sertifikasi</a></td>
    ...
    <script>
        document.querySelectorAll('[id^="btnDelete"]').forEach(button => {
            button.addEventListener('click', function() {
                ...
                deleteLink.setAttribute('href', '/delete-riwayat-sertifikasi/' + itemId);
            });
        });

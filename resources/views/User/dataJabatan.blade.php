        ...
        <div class="border rounded-lg p-3 w-full bg-white mb-3" id="containerAdd">
            <form id="addForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <!-- Tingkat Pelatihan -->
                    <div>
                        <label for="lokasi_jabatan" class="block mb-2 text-sm font-medium text-gray-900">Lokasi
                            Jabatan<span class="text-red-400">*</span></label>
                        <select id="lokasi_jabatan" name="lokasi_jabatan"
                    ...
                    <!-- Rayon -->
                    <div>
                        <label for="jabatan" class="block mb-2 text-sm font-medium text-gray-900">Jabatan<span class="text-red-400">*</span></label>
                    ...
                    <!-- Sertifikat -->
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 " for="file_input">SK Jabatan</label>
                        <input
                            class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50"
                            aria-describedby="file_input_help" id="file_input" type="file" name="sk_jabatan"
                    ...
                    <tbody id="adminTableBody">
                        @foreach ($jabatan as $item)
                            <tr class="odd:bg-white">
                                ...
                                <td class="px-6 py-4 text-center">{{ $item->lokasi_jabatan }}</td>
                                <td class="px-6 py-4 text-center">{{ $item->jabatan }}</td>
                                <td class="px-6 py-4 text-center"><a target="#"
                                        href="/sk-jabatan/{{ $item->id }}"
                                        class="text-violet-500 hover:text-violet-600 hover:underline">SK Jabatan</a></td>
    ...
    <script>
        document.querySelectorAll('[id^="btnDelete"]').forEach(button => {
            button.addEventListener('click', function() {
                ...
                deleteLink.setAttribute('href', '/delete-data-jabatan/' + itemId);
            });
        });

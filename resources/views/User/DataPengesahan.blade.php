                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <div>
                        ...
                        <select id="tingkat" name="tingkat"
                            ...
                            <option value="Tingkat I" {{ old('tingkat') == 'Tingkat I' ? 'selected' : '' }}>Tingkat I
                            </option>
                            <option value="Tingkat II"{{ old('tingkat') == 'Tingkat II' ? 'selected' : '' }}>Tingkat II
                            </option>
                            <option value="Tingkat III" {{ old('tingkat') == 'Tingkat III' ? 'selected' : '' }}>Tingkat III
                            </option>
                        </select>
                        @error('tingkat')
                    ...
                    <div>
                        <label for="cabang" class="block mb-2 text-sm font-medium text-gray-900">Cabang<span
                                class="text-red-400">*</span></label>
                        <input type="text" id="cabang" name="cabang"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="Cabang" value="{{ old('cabang') }}" />
                        @error('cabang')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="tahun" class="block mb-2 text-sm font-medium text-gray-900">Tahun Pengesahan<span
                                class="text-red-400">*</span></label>
                        <input type="number" min="1900" max="2099" step="1" id="tahun" name="tahun"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="tahun pengesahan" value="{{ old('tahun') }}" />
                        @error('tahun')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            aria-describedby="file_input_help" id="file_input" type="file" name="sertifikat_pengesahan"
                            accept="application/pdf">
                        <p class="mt-1 text-sm text-gray-500" id="file_input_help">PDF (MAX 3mb).</p>
                        @error('sertifikat_pengesahan')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
                        ...
                        @foreach ($pengesahan as $item)
                            <tr class="odd:bg-white">
                                ...
                                <td class="px-6 py-4 text-center"><a target="#"
                                        href="/sertifikat-pengesahan/{{ $item->id }}"
                                        class="text-violet-500 hover:text-violet-600 hover:underline">Sertifikat Pengesahan</a></td>
    ...
    <script>
        document.querySelectorAll('[id^="btnDelete"]').forEach(button => {
            button.addEventListener('click', function() {
                ...
                deleteLink.setAttribute('href', '/delete-data-pengesahan/' + itemId);
            });
        });
